...
class BioticGuardian: Monster {
    ...
    override func setupMesh() {
        ...
        assert(masterClayMesh.vertices.count > 0, "BioticGuardian vertex count is 0")
    ...
    override func setupTexture() {
        if getRandomMicro().nextBool()
        {
            ...
            self.name = getCave().texturePalette.mainNaturalWallName + " Guardian"
        }
        else
        {
            self.texture = getCave().texturePalette.mainNaturalFloor
            self.name = getCave().texturePalette.mainNaturalFloorName + " Guardian"
        }
    }
